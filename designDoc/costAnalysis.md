...
***
***
**單一 Agent 月度用量預測 (10 位日活躍用戶)**
- 一位「日活躍用戶」平均每天會與 Agent 進行 15 次互動 (Function Invocations)。
- 每月以 30 天計算
...
|資源類別  |  計費項目 (Billing Item)  |  免費額度可支持的規模 (Agent 數量 / DAU)  |  第 50 個 Agent 的邊際成本  |  第 100 個 Agent 的邊際成本  |  第 200 個 Agent 的邊際成本  |  第 500 個 Agent 的邊際成本  |  第 1000 個 Agent 的邊際成本|
| -------- | ------- | -------- | -------- | -------- | ------- | -------- | -------- |
|邊緣網絡  |  Edge Requests  |  ~108 個 / ~3,000  |  $0  |  $0  |  $0.13  |  $0.19  |  $0.23|
||Fast Data Transfer  |  ~295 個 / ~11,000  |  $0  |  $0  |  $0  |  $0.23  |  $0.28|
|無伺服器函數  |  Function Duration  |  ~114 個 / ~3,200  |  $0  |  $0  |  $0.86  |  $1.13  |  $1.38|
||Function Invocations  |  ~25 個 / ~564  |  $0.02  |  $0.04  |  $0.07  |  $0.13  |  $0.18|
||Fast Origin Transfer  |  ~363 個 / ~14,000  |  $0  |  $0  |  $0  |  $0.02  |  $0.02|
|內容與快取  |  Blob Storage  |  ~222 個 / ~7,800  |  $0  |  $0  |  $0  |  $0.006  |  $0.007|
||  Neon DB  |  0 個  |  ~$2.41  |  ~$2.41  |  ~$2.41  |  ~$2.41  |  ~$2.41|
|外部服務  |LLM API  |  0 個  |  ~$6.08 至 $9.77  |  ~$6.08 至 $9.77  |  ~$6.08 至 $9.77  |  ~$6.08 至 $9.77  |  ~$6.08 至 $9.77|
|總計  |  總邊際成本  |    |  ~$8.51 至 $12.20  |  ~$8.53 至 $12.22  |  ~$9.55 至 $13.24  |  ~$10.27 至 $13.96  |  ~$10.66 至 $14.35|
...
***
***
**單一 Agent 月度用量預測 (10 位日活躍用戶, 基於 AWS)**
...
| 資源類別 | 計費項目 (Billing Item) | 免費額度可支持的規模 (Agent 數量 / DAU) | 第 50 個 Agent 的邊際成本 | 第 100 個 Agent 的邊際成本 | 第 200 個 Agent 的邊際成本 | 第 500 個 Agent 的邊際成本 | 第 1000 個 Agent 的邊際成本 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 邊緣與運算 | CloudFront/API Gateway Requests | ~21 個 / ~470 | $0.05 | $0.10 | $0.18 | $0.35 | $0.57 |
| | Lambda Invocations | ~39 個 / ~980 | $0.01 | $0.01 | $0.02 | $0.04 | $0.06 |
| | Lambda Duration | ~34 個 / ~830 | $0.19 | $0.38 | $0.66 | $1.29 | $2.10 |
| | CloudFront Data Transfer | ~830 個 / ~38,000 | $0 | $0 | $0 | $0.12 | $0.19 |
| 儲存與資料庫 | S3 Storage | ~222 個 / ~7,800 (首年) | $0 | $0 | $0 | $0.001 | $0.001 |
| | Aurora DB | 0 個 | ~$1.00 | ~$1.00 | ~$1.00 | ~$1.00 | ~$1.00 |
| 外部服務 | LLM API | 0 個 | ~$6.08 至 $9.77 | ~$6.08 至 $9.77 | ~$6.08 至 $9.77 | ~$6.08 至 $9.77 | ~$6.08 至 $9.77 |
| 總計 | 總邊際成本 | | ~$7.33 至 $11.02 | ~$7.57 至 $11.26 | ~$8.94 至 $12.63 | ~$10.85 至 $14.54 | ~$12.00 至 $15.69 |
...
### 1. 伺服器託管 (Server Hosting)

此服務讓創作者能將他們的 Agent 部署在我們的基礎設施上，對外提供服務。

#### 定價計算方式與建議價格
- **開通費 (`enable_hosting`)**: **$10 (一次性費用 / 每 Agent)**
- **運行費 (資源包)**: **$15 (手動購買 / 30天有效期)**
  - 包含 `100 DAU` 的服務能力、`1 GB` 儲存空間、`2,000 GB-秒` 函數運算。

#### 對應成本分析
- **核心成本**: 此定價直接對應本文件中的「AI Agent 方案成本分析」和「ROM AI 方案成本分析」章節。主要覆蓋以下項目的邊際成本：
  - **無伺服器函數 (Vercel Function Duration / AWS Lambda Duration)**: Agent 運行所需的核心運算資源。
  - **資料庫 (Neon DB / Aurora DB)**: Agent 數據的持續性存儲與讀寫，這是主要的固定成本之一。
  - **內容與快取 (Vercel Blob / AWS S3, ISR)**: 存儲 Agent 生成的產物和提供前端內容。
  - **網路傳輸 (Fast Data Transfer / CloudFront Data Transfer)**: 使用者與 Agent 互動產生的帶寬費用。
- **成本依據**: 根據本文件的分析：
  - 在 **Vercel** 模型下，排除 LLM 成本後的純基礎設施成本 (Neon DB, Functions 等) 約為 **$2-4**。
  - 在 **AWS** 模型下，根據規模化成本表格，純基礎設施成本是一個動態區間：起初約為 **$1.25**，並會隨著平台規模擴大而增長，在千級 Agent 規模時可達近 **$4**。
不論在哪種模型下，成本主要都由 **LLM API** 和 **資料庫** 驅動。總邊際成本（包含 LLM）預計在 **$8-15** 的範圍內，具體取決於供應商和平台規模。

#### 定價理由
- **$10 開通費**: 設定一個較低的門檻，用於覆蓋每個 Agent 進行初始配置的固定資源成本和人力成本
- **$15 資源包**:
  - **覆蓋成本與利潤**: 此價格高於我們預估的最高基礎設施邊際成本 ($2-4)，並為更大規模 (100 DAU) 的使用留下充足的緩衝空間，確保業務的盈利能力。
  - **簡化心智模型**: 將複雜的基礎設施成本（CPU、帶寬、儲存）打包成一個創作者易於理解的「資源包」，提供了可預測的月度開銷，同時避免了使用者反感的「自動訂閱」。
  - **激勵措施**: 創作者需要主動購買資源包，這促使他們更積極地運營自己的 Agent 以創造價值，支付託管費用。

---

### 2. AI 運行功能 (AI Running)

此服務讓 Agent 能夠調用大型語言模型 (LLM) 來執行複雜的智能任務。

#### 定價計算方式與建議價格
- **開通費 (`enable_ai`)**: **$5 (一次性費用 / 每 Agent)**
- **運行費 (`ai_credit`)**: 預先購買的點數包。
  - **$5** = 2,000,000 點
  - **$10** = 5,000,000 點 (20% 優惠)
  - **$20** = 12,000,000 點 (33% 優惠)

#### 對應成本分析
- **核心成本**: 直接對應本文件中的「AI model usage」章節。這是平台最大且最不穩定的可變成本。
- **成本依據**: 以 OpenAI gpt-4o-mini 為例，其輸出成本為 **$0.60 / 百萬 Tokens**。一個 10 DAU 的 Agent 每月 LLM 成本預估為 **~$6-10**。

#### 定價理由
- **$5 開通費**: 作為一個低門檻的付費轉化點，引導使用者進入 AI 點數的消費體系。
- **點數包定價**:
  - **保證利潤率**: 點數定價約為底層 LLM 成本的 **3-4 倍** (例如，以 $0.6 的成本獲得百萬 token，以約 $2.0-$2.5 的價格賣出等量點數)。
    1.  **吸收成本波動**: LLM 提供商的價格可能隨時變動。
    2.  **覆蓋運營成本**: API 調用、監控、日誌記錄等隱性成本。
    3.  **產生核心利潤**。
  - **鼓勵儲值**: 透過提供階梯式折扣，鼓勵使用者進行更大額度的單次購買，增加現金流並提高使用者黏性。

---

### 3. Web3 整合服務 (NFT & ERC-20)

#### 定價計算方式與建議價格
- **NFT 授權合約創建**:
  - **開通費**: **$19 (初期推廣價 / 每 Agent)**
  - **運行費**: **5%** 的平台佣金 (從 NFT 初級銷售收入中抽取)。
- **ERC-20 股權代幣創建**:
  - **開通費**: **$49 (初期推廣價 / 每 Agent)**
  - **運行費**: **5%** 的平台佣金 (從聯合曲線交易手續費中抽取)。

#### 對應成本分析
- **核心成本**: 這部分成本主要是 **研發與維護成本**，而非直接的基礎設施運行成本。包括：
  - 開發和審計安全可靠的智慧合約工廠。
  - 維護 Web3 基礎設施（節點、索引器等）的穩定性。
  - 處理鏈上交易的複雜性。

#### 定價理由
- **降低入門門檻**: 在平台初期，我們希望最大限度地吸引創作者體驗核心的 Web3 商業模式。**$19** 和 **$49** 的低價開通費旨在作為**推廣策略**，鼓勵大量創作者嘗試將其 Agent 代幣化，快速建立生態。
- **價值捕獲後移**: 我們將主要的價值捕獲點後移至 **5% 的運行佣金**。這意味著平台的收入增長與創作者的成功更緊密地綁定。我們更關心 Agent 長期的成功，而非初期的一次性收入。
- **利益綁定**: 佣金模式將平台的收入與創作者的成功直接掛鉤。只有當 Agent 成功產生收入時，平台才能獲得可持續的回報，這創造了一個雙贏的生態系統。$49 的 ERC-20 費用高於 NFT，因為其智慧合約（特別是聯合曲線）的複雜性和風險更高。
